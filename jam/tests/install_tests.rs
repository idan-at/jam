...
use common::*;
use jam::cli_options::{CliOptions, Command, Install};
use jam::errors::JamError;
use jam::run;
use jam_test_utils::async_helpers::*;
...
async fn fails_on_missing_manifest_file() {
    given_manifest_file_does_not_exist(|path| async move {
        ...
        let expected = Err(JamError::new(String::from(
            "Couldn't find root directory. Make sure jam.json exists",
        )));
