...
impl CacheFactory {
    ...
    pub fn create_cache(&self, cache_name: &str) -> Result<Cache, JamCacheError> {
...
impl Cache {
    ...
    pub fn set(&self, key: &str, value: &[u8]) -> Result<PathBuf, JamCacheError> {
        let key_path = self.cache_dir.join(sanitize_package_name(key));
